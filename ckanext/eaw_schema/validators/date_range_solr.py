...
the DateRangeField specification of SOLR 8.2.
See https://solr.apache.org/guide/8_2/working-with-dates.html
...
class SolrDaterange:
    ...
    regex_explicit_range = re.compile(
        r"^\[(?P<start>[\-TZ:\.\d]{4,}|\*)" + r" TO (?P<end>[\-TZ:\.\d]{4,}|\*)\]$"
    )
    ...
    def _check_month_day_validity(datedict):
        def noleap(y):
            return y % 4 != 0 or (intyear % 100 == 0 and intyear % 400 != 0)
    ...
    def _check_implicit_range(cls, rangestr):
        ...
        if matchres is None:
            raise Invalid("{}: not a valid DateRange - field".format(rangestr))
        else:
    ...
    def _check_time_direction(cls, start, end):
        ...
        if [start, end] != sorted([start, end]):
            raise Invalid(
                "You inverted the arrow of time!\n" + "{} < {}".format(end, start)
            )
